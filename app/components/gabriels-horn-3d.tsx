...
export function GabrielsHorn3D() {
  ...
  useEffect(() => {
    ...
    // label positions are computed by projecting real 3D anchor points through
    // the actual camera, not hand-picked CSS percentages - this keeps them
    // genuinely locked to the axes/curve regardless of zoom or resize, which
    // a fixed percentage could never guarantee
    const yLabelAnchor = new THREE.Vector3(-0.18, 1.55, 0);
    const xLabelAnchor = new THREE.Vector3(X_MAX + 0.9, -0.35, 0);
    const curveLabelAnchor = new THREE.Vector3(profilePoints[0].y - 0.25, profilePoints[0].x + 0.4, 0);

    const worldToPercent = (localPoint: THREE.Vector3) => {
      const world = group.localToWorld(localPoint.clone());
    ...
    const updateLabels = () => {
      setLabelPos({
        y: worldToPercent(yLabelAnchor),
        x: worldToPercent(xLabelAnchor),
        curve: worldToPercent(curveLabelAnchor)
      });
    };
    updateLabels();
  ...
  return (
    ...
    >
      <div className="horn3d-frame">
        ...
        <span className="horn3d-label horn3d-label-y" style={{ top: `${labelPos.y.top}%`, left: `${labelPos.y.left}%` }}>y</span>
        <span className="horn3d-label horn3d-label-x" style={{ top: `${labelPos.x.top}%`, left: `${labelPos.x.left}%` }}>x</span>
        <span className="horn3d-label horn3d-label-curve" style={{ top: `${labelPos.curve.top}%`, left: `${labelPos.curve.left}%` }}>y = 1/x</span>
      </div>
      <button
