 ...
 * (y = 1/x spun 360deg around the x-axis, x=1 to infinity).
 *
 * Two things do the work of selling "3D, not a flat curve":
 * 1. The whole drawing sits inside a real CSS 3D `perspective` scene and is
 *    rotated in X/Y (not a flat 2D `rotate`), so it reads as a tilted plane
 *    viewed at an angle, like a photo tipped away from the camera.
 * 2. Each cross-section ring is traced by a radius spoke that visibly
 *    sweeps a full turn in sync with the ring's own stroke drawing in
 *    (both driven by the same 0-100 `pathLength`), so it looks like the
 *    circle is being generated by rotation, not just growing/popping in.
 *    The mouth ring keeps a slow ambient spin afterwards so the "this
 *    sweeps around" idea persists rather than being a one-off intro.
 */
export function DrawnHorn() {
  ...
  const rings = [
    { cx: 30, cy: 46, rx: 13, ry: 34, delay: 0.15 },
    { cx: 84, cy: 46, rx: 9, ry: 19, delay: 0.5 },
    { cx: 140, cy: 46, rx: 6, ry: 9, delay: 0.85 }
  ];
  ...
  return (
    <div className="horn-scene">
      ...
      >
        ...
        {rings.map((r, i) => (
          <g key={i} className={`horn-ring-group horn-ring-group-${i + 1}`} style={{ "--ring-delay": `${r.delay}s` } as React.CSSProperties}>
            <ellipse
              cx={r.cx}
              cy={r.cy}
              rx={r.rx}
            ...
            <line
              ...
              x2={r.cx}
              y2={r.cy - r.ry}
              className={`horn-spoke ${i === 0 ? "horn-spoke-mouth" : "horn-spoke-intro"}`}
            />
          </g>
        ))}
